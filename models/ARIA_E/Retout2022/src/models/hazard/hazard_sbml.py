...
def create_hazard_model(params):
    # Create SBML document
    document = libsbml.SBMLDocument(3, 2)
    model = document.createModel("Hazard_Model")
    ...
    math_ast = libsbml.parseL3Formula("Ke0 * (C - Ce)")
    ...
    math_ast = libsbml.parseL3Formula(
        "ln(BSAPOE4_carrier) + (Emax * C)/(C + EC50) * (T50^gamma)/(T50^gamma + time^gamma)"
    )
...
def main(params):
    document = create_hazard_model(params)
    ...
    if document.getNumErrors() > 0:
        ...
        save_model(document, "hazard_sbml.xml")
        print("Hazard Model saved successfully!")
